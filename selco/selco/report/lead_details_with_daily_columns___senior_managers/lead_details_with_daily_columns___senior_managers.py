...
def get_lead_details(filters):
    ...
    ses_details = frappe.db.sql("""SELECT branch,parent_sales_person,name,contact_number,designation FROM `tabSales Person` WHERE designation IN ("Senior Manager Energy Solutions") AND show_in_daily_report = 1 ORDER BY branch ASC""",as_list = True )
